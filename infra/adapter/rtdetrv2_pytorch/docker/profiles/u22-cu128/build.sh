...
print_usage() {
  cat <<'EOF'
Usage: infra/adapter/rtdetrv2_pytorch/docker/profiles/u22-cu128/build.sh [options]
...
Build and start the Ubuntu 22.04 + CUDA 12.8 profile:
  ...
  docker compose -f docker/profiles/u22-cu128/docker-compose.yml \
    -f infra/adapter/rtdetrv2_pytorch/docker/profiles/u22-cu128/docker-compose.yml up -d --build runner
...
CONTAINER_NAME="nn-framework-rtdetrv2-model"
...
docker compose \
  -f docker/profiles/u22-cu128/docker-compose.yml \
  -f infra/adapter/rtdetrv2_pytorch/docker/profiles/u22-cu128/docker-compose.yml \
