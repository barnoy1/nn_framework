...
print_usage() {
  cat <<'EOF'
Usage: infra/adapter/rf_detr/docker/profiles/u20-cu128/build.sh [options]
...
Build and start the Ubuntu 20.04 + CUDA 12.8 compatibility profile:
  ...
  docker compose -f docker/profiles/u20-cu128/docker-compose.yml \
    -f infra/adapter/rf_detr/docker/profiles/u20-cu128/docker-compose.yml up -d --build runner
...
CONTAINER_NAME="nn-framework-rf-detr-model-u20-cu128"
...
docker compose \
  -f docker/profiles/u20-cu128/docker-compose.yml \
  -f infra/adapter/rf_detr/docker/profiles/u20-cu128/docker-compose.yml \
  up -d --build --force-recreate runner

echo "Started RF-DETR compatibility container: rf-detr-model:u20-cu128"
