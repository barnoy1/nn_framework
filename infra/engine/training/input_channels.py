...
def expected_model_input_channels(model) -> Optional[int]:
    first_conv_weight = next((parameter for parameter in model.parameters() if getattr(parameter, "ndim", 0) == 4), None)
    if first_conv_weight is None:
        return None
    return int(first_conv_weight.shape[1])


def align_images_to_model_input_channels(*, images: torch.Tensor, model) -> torch.Tensor:
