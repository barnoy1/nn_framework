...
class CommonConfig(BaseModel):
    ...
    def validate_score_threshold(cls, value: float) -> float:
        ...
        if numeric < 0.0 or numeric > 1.0:
            raise ValueError("engine.execution.score_threshold must be in [0, 1]")
        return numeric
...
class ExecutionConfig(BaseModel):
    ...
    tracking: VisualizationConfig = Field(
        default_factory=VisualizationConfig,
        validation_alias=AliasChoices("tracking", "visualization"),
    )
    ...
    @property
    def run_id(self) -> str:
        return Path(self.common.output_dir).name

    @property
    def output_base(self) -> Path:
        # Layout contract: output_dir == <base>/runs/<run_id>. Fall back to the
        # output_dir itself when it is not nested under a runs/ parent.
        run_dir = Path(self.common.output_dir)
    ...
    def mlflow_dir(self) -> Path:
        return self.output_base / "mlflow"

    @property
    def tensorboard_dir(self) -> Path:
        return self.output_base / "tensorboard" / self.run_id
