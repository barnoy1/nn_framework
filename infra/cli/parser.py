...
def add_common_arguments(
    target_parser: argparse.ArgumentParser, defaults: dict[str, Any]
) -> None:
...
def register_inference_onnx_parser(
    subparsers: argparse._SubParsersAction, defaults: dict[str, Any]
) -> None:
    infer_onnx_parser = subparsers.add_parser(
        "inference-onnx", help="Run ONNX inference"
    )
    ...
    infer_onnx_parser.add_argument(
        "--input-dir", type=str, default=defaults["input_dir"]
    )
...
def register_export_coco_rle_parser(
    ...
    export_parser = subparsers.add_parser(
        "export-coco-rle", help="Convert Supervisely rectangles to COCO RLE"
    )
    export_parser.add_argument("--config", type=str, required=True)
    export_parser.add_argument(
        "--dataset-conf", type=str, default=defaults["dataset_conf"]
    )
    export_parser.add_argument(
        "--experiment-conf", type=str, default=defaults["experiment_conf"]
    )
    export_parser.add_argument(
        "--dataset_root", type=str, default=defaults["dataset_root"]
    )
