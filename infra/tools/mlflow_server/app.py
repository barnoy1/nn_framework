...
def run(args: Namespace) -> None:
    ...
    tracking_dir = build_aggregate_tracking_dir(
        root_candidate, source_tracking_dirs, args.sqlite_db_name
    )
    ...
    if is_port_in_use(args.host, selected_port):
        ...
        if not reused:
            ...
            logger.warning(
                "Port {} is busy, using {} instead.", selected_port, fallback
            )
    ...
    if wait_for_service(host=args.host, port=selected_port, timeout_seconds=15.0):
        ...
        logger.warning(
            "MLflow UI is still starting. Try opening {} in a few seconds.", url
        )
